...
class LinearModel(tf.keras.Model):
    ...
    def __init__(self, n_inputs, timesteps):
        ...
        block = int(n_inputs/timesteps)

        for timestep in range(timesteps):
            for index in range(block):
                index = index + timestep * block
                name = 'input' + str(index)
                inputs.append(Input(shape=(1,), name=name))
            x.append(concatenate(inputs[(timestep * block):((timestep + 1) * block)]))

        neurons = int((n_inputs / timesteps) * 20)

        for timestep in range(timesteps):
            for index in range(3):
                x[timestep] = Dense(neurons, activation='relu')(x[timestep])
        ...
        main_output = Dense(1, activation='linear', name='main_output')(y)
