...
class LinearModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        x = concatenate([rain_input, paderno_input, niguarda_input, niguarda_input_minus_1])
        ...
        self.model = Model(inputs=[rain_input, paderno_input, niguarda_input, niguarda_input_minus_1], outputs=main_output)
    ...
    def fit(self, x, y, epochs, verbose, validation_split, validation_data):
        ...
        early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=15, verbose=0,
                                                      mode='auto', baseline=None, restore_best_weights=True)

        history_file = self.model.fit(x=x, y=y, batch_size=None, epochs=epochs, verbose=verbose, callbacks=[reducelr,
                       early_stop, history], validation_split=validation_split, validation_data=validation_data, shuffle=True)
